...
export const BrewsRelease = ({
  ...
  return (
    <div className="relative flex-auto h-fit md:h-full md:w-full lg:w-full shadow-sm bg-[#f0f0f0] dark:bg-[#1f1f1f] rounded-sm shadow-md rounded-lg">
      <div className="p-4">
        <div className="">
          {image && (
            <Image
            ...
            <span className="flex flex-auto justify-end">
              {showDelete && (
                ...
                >
                  <motion.div whileTap={{ scale: 0.85 }} className="w-4">
                    {isDeleting === true ? (
                      <FontAwesomeIcon
                        className="flex-auto w-full mx-auto animate-spin"
                        icon={faSpinner}
                        style={{ color: `${iconColor}` }}
                      />
                    ) : (
                      <FontAwesomeIcon
                        className="flex-auto w-full mx-auto"
                        icon={faTrash}
                        style={{ color: `${iconColor}` }}
                      />
                    )}
              ...
              <Link href={`${fb}`}>
                <a target="_blank" className="px-2">
                  <motion.div
                    whileTap={{ scale: 0.85 }}
                    className="w-8 h-8 bg-[#4267B2] flex items-center justify-center rounded-full"
                  >
                    <svg
                      className="fill-white h-4/6 w-4/6"
              ...
              <Link href={`${ig}`}>
                <a target="_blank" className="pr-2">
                  <motion.div
                    whileTap={{ scale: 0.85 }}
                    className="w-8 h-8 bg-[#C13584] flex items-center justify-center rounded-full"
                  >
                    <svg
                      className="fill-white h-4/6 w-4/6"
                      xmlns="http://www.w3.org/2000/svg"
